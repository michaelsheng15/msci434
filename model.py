...
model.addConstr(gp.quicksum(s_f[f] for f in suppliers) >= gp.quicksum(b_r[r] for r in retailers))

# Amount shipped from supplier to warehouse is the total amount produced
for f in suppliers:
    model.addConstr(gp.quicksum(x_sw[f, w] for w in warehouses) == s_f[f])

# Amount produced of each supplier cannot exceed maximum capacity of supplier
for f in suppliers:
    model.addConstr(s_f[f] <= m_s[f])

# Amount shipped is equal to the demand of each retailer
for r in retailers:
    model.addConstr(gp.quicksum(x_wr[w, r] for w in warehouses) == b_r[r])

# Amount shipped into each warehouse is equal to the amount shipped out of each node
for w in warehouses:
    model.addConstr(gp.quicksum(x_sw[s, w] for s in suppliers) == gp.quicksum(x_wr[w, r] for r in retailers))

# Amount shipped into each warehouse is less than or equal to maximum capacity of the warehouse
for w in warehouses:
    model.addConstr(gp.quicksum(x_wr[w, r] for r in retailers) <= m_td[w])
...
#RESULTS
print("\n\nRESULTS:")
print("Model Solution Duration: ", length, "seconds!")
